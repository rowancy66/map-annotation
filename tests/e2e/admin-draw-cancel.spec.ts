...
test('修改地图公开状态后会写入审计记录', async ({ page }) => {
  const appRequest = page.context().request;

  await ensureLoggedIn(appRequest);
  const mapProject = await createMap(appRequest);

  const updateResponse = await appRequest.put(`/api/maps/${mapProject.id}`, {
    data: {
      settings: {
        isPublic: false,
        showNames: true,
  ...
  const auditResponse = await appRequest.get(`/api/maps/${mapProject.id}/audits`);
  expect(auditResponse.ok(), await auditResponse.text()).toBeTruthy();
  const auditData = await auditResponse.json() as MapAuditResponse;

  expect(auditData.audits.length).toBeGreaterThan(0);
  expect(auditData.audits[0]).toMatchObject({
    map_id: mapProject.id,
    action: 'settings_updated',
  });
  expect(auditData.audits[0].previous_value).toContain('"isPublic":true');
